...
int Application6::Initialize()
{
	...
	GzToken		nameListAntialias[2];
	GzPointer	valueListAntialias[2];
	...
	for (int i = 0; i < AAKERNEL_SIZE; ++i)
	{
		...
		*/
		...
		nameListAntialias[0] = GZ_AASHIFTX;
		valueListAntialias[0] = (GzPointer)&AAFilter[i][0];

		nameListAntialias[1] = GZ_AASHIFTY;
		valueListAntialias[1] = (GzPointer)&AAFilter[i][1];

		status |= GzPutAttribute(m_pRender[i], 2, nameListAntialias, valueListAntialias);
...
int Application6::Render() 
{
	...
	for (int i = 0; i < m_pDisplay[0]->xres * m_pDisplay[0]->yres; ++i)
	{
		...
		{
			float weight = AAFilter[j][2];
			pixel.red += weight * m_pDisplay[j]->fbuf[i].red;
			pixel.green += weight * m_pDisplay[j]->fbuf[i].green;
			pixel.blue += weight * m_pDisplay[j]->fbuf[i].blue;
			//pixel.alpha += weight * m_pDisplay[j]->fbuf[i].alpha;
			pixel.z += weight * m_pDisplay[j]->fbuf[i].z;
			if (pixel.red == 0 && pixel.green == 0 && pixel.blue == 0)
			{
				int a = 0;
			}
		}
		// Done with this pixel for all displays
		// Set first display with final output pixel
